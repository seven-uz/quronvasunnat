...
<main>
					...
					<div class="card w-100">
						...
						<div class="card-header p-2 d-flex justify-content-between">
							...
							<div class="randBtns">
								...
								<i onclick="document.getElementById('s<?= $suraRow['ns'] ?>o<?= $suraRow['no'] ?>').pause();document.getElementById('s<?= $suraRow['ns'] ?>o<?= $suraRow['no'] ?>').loop = false;" class="fas fa-pause d-none" id="pauseRS" data-toggle="tooltip" data-placement="top" title="<?php echo word('Qotirish') ?>"></i>
								<i onclick="document.getElementById('s<?= $suraRow['ns'] ?>o<?= $suraRow['no'] ?>').play();document.getElementById('s<?= $suraRow['ns'] ?>o<?= $suraRow['no'] ?>').loop = true;" class="fas fa-sync-alt" id="loopRS" data-toggle="tooltip" data-placement="top" title="<?php echo word('Qayta-qayta eshitish') ?>"></i>
								<i onclick="document.getElementById('s<?= $suraRow['ns'] ?>o<?= $suraRow['no'] ?>').pause();document.getElementById('s<?= $suraRow['ns'] ?>o<?= $suraRow['no'] ?>').currentTime=0" class="fas fa-stop d-none" id="stopRS" data-toggle="tooltip" data-placement="top" title="<?php echo word('To‘xtatish') ?>"></i>
								...
								<a href="https://t.me/share/url?url=<?= $_SERVER['SERVER_NAME'] ?>/quron?id=<?= $suraRow['no'] ?>&text=<?= $suraRow['textar'] . '<br>' . $suraRow['mano'] ?>"><i class="fas fa-share-alt" id="shareAyah" data-toggle="tooltip" data-placement="top" title="<?php echo word('Do‘stlar bilan baham ko‘rish') ?>"></i></a>
								<i onclick="copytext('#copyRS<?= $suraRow['id'] ?>')" class="fas fa-copy" data-toggle="tooltip" data-placement="top" title="<?php echo word('Matnni ko‘chirib olish') ?>"></i>
								<p style="display:none" id="copyRS<?= $suraRow['id'] ?>"><i style="text-align:right">
										<?php echo $suraRow['textar'] . "</i>\n" . word('Maʼnosi') . ": " . $suraRow['mano'] . "\n[" . $suraRow['title'] . " surasi " . $suraRow['no'] . "-oyat]"; ?>
								</p>
							</div>
						</div>
						<div class="card-body" id="">
							...
							<p class="mano indent-20"><b>
									<?php echo mb_strimwidth(word($suraRow['mano']), 0, 500, '...') ?></b><br>
								<a href="quron?sura=<?= $suraRow['ns'] ?>#<?= $suraRow['no']; ?>">[
									<?php echo word($suraRow['title']) . ' ' . word('surasi') . ' ' . $suraRow['no'] . '-' . word('oyat') ?>]</a></p>
						</div>
					</div>
				</div>
					...
					<div class="card w-100">
						...
						<div class="card-header d-flex justify-content-between">
							...
							<div class="randBtns">
								...
								<i onclick="document.getElementById('d<?= $duoRow['id'] ?>').play();document.getElementById('d<?= $duoRow['id'] ?>').loop = true;" class="fas fa-sync-alt" id="loopRD" data-toggle="tooltip" data-placement="top" title="<?php echo word('Qayta-qayta eshitish') ?>"></i>
								<i onclick="document.getElementById('d<?= $duoRow['id'] ?>').pause();document.getElementById('d<?= $duoRow['id'] ?>').currentTime=0" class="fas fa-stop d-none" id="stopRD" data-toggle="tooltip" data-placement="top" title="<?php echo word('To‘xtatish') ?>"></i>
								...
								<i onclick="copytext('#copyRD<?= $duoRow['id'] ?>')" class="fas fa-copy" data-toggle="tooltip" data-placement="top" title="<?php echo word('Matnni ko‘chirib olish') ?>"></i>
								<p style="display:none" id="copyRD<?= $duoRow['id'] ?>"><i style="text-align:right">
										<?php echo $duoRow['textar'] . "</i>\n" . word($duoRow['text']) . "\n\n" . word('Manosi') . ": " . word($duoRow['mano']) ?>
								</p>
							</div>
						</div>
						<div class="card-body">
							<h5 class="card-title"><a href="duo?id=<?= $duoRow['id'] ?>">
									<?php echo word($duoRow['title']) ?></a></h5>
							<p class="randDuo"><?= $duoRow['textar'] ?></p>
							<p class="mano indent-20"><b>
									<?php echo word($duoRow['text']) ?></b></p>
							<p class="mano indent-20">
								<?php echo word('Ma’nosi') ?>: <b>
									<?php echo word($duoRow['mano']) ?></b></p>
						</div>
					</div>
				</div>
				<div class="col-md-4">
							...
							<div class="card-header d-flex justify-content-between">
								...
								<div class="randBtns">
									...
									<p style="display:none" id="copyRSun<?= $sunnatRow['id'] ?>"><i>
											<?php echo word($sunnatRow['title']) . "</i>\n\n" . word($sunnatRow['text']) ?>
									</p>
									<a href="sunnat?id=<?= $sunnatRow['id'] ?>"><i class="fas fa-link" data-toggle="tooltip" data-placement="top" title="<?php echo word('To‘liq ko‘rish') ?>"></i></a>
								</div>
							</div>
							<div class="card-body">
								<h5 class="card-title">
									<?php echo word($sunnatRow['title']) ?>
								</h5>
								<p class="mano"><b>
										<?php echo word($sunnatRow['text']) ?></b></p>
							</div>
							<?php }else{ ?>
								<div class="card-header d-flex justify-content-between">
									<span class="btn btn-transparent disabled mr-5"><?php echo word('Tasodifiy sunnat') ?></span>
								</div>
								<div class="card-body">
									<h5 class="card-title"><?php echo word('Ma‘lumot yo‘q') ?></h5>
								</div>
						<?php } ?>
					</div>
				</div>
	...
	<section class="section" id="aboutKoran">
		<div class="container">
			<h4 class="text-center h2 d-flex justify-md-content-center justify-content-between align-items-center">
				<?php echo word('Qur’on haqida') ?>...
				<span style="font-size:16px">
					<a href="quron">
						<span class="d-none d-md-block">
							<?php echo word('To‘liq o‘qish')?></span>
			...
			<div class="section-body text-justify indent-20">
				...
				<div class="item">
					<h5>
						<?php echo word($rowi['title']) ?>
					</h5>
					<p>
						<?php echo word($rowi['text']) ?>
					</p>
				</div>
				<div class="item">
					<h6></h6>
	...
	<section id="asmaulhusna">
		<div class="container">
			<h4 class="h2 d-flex justify-md-content-center justify-content-between align-items-center">
				<?php echo word('Asma ul-Husna') ?>
				<span style="font-size:16px">
					<a href="asmaulhusna">
						<span class="d-none d-md-block">
							<?php echo word('Barchasini ko‘rish')?></span>
						<i class="fas fa-angle-double-right d-md-none"></i>
					</a>
				</span>
			</h4>
		...
		<div class="owl-carousel" id="first-owl">
			<?php
			$asmaulhusna = mysqli_query($db, "SELECT * FROM asmaulhusna WHERE id IN ($randomAuH) LIMIT 30");
			while($row = $asmaulhusna->fetch_assoc()){
				echo '<a href="asmaulhusna?=#'.$row['title'].'">
					<div class="item">
	...
	<!-- <section class="section" id="lastDuo">
		<div class="container">
			<h4 class="h2 d-flex justify-md-content-center justify-content-between align-items-center">
				<?php echo word('So‘ngi qo‘shilgan duolar') ?>
				<span style="font-size:16px">
					<a href="duo">
						<span class="d-none d-md-block">
							<?php echo word('Barchasini ko‘rish')?></span>
	...
	<section class="section" id="lastHadis">
		<div class="container">
			<h4 class="h2 d-flex justify-md-content-center justify-content-between align-items-center">
				<?php echo word('So‘ngi qo‘shilgan hadislar') ?>
				<span style="font-size:16px">
					<a href="hadis">
						<span class="d-none d-md-block">
							<?php echo word('Barchasini ko‘rish')?></span>
			...
			<div class="row">
				<?php $lastHadis = mysqli_query($db, "SELECT * FROM hadislar ORDER BY id DESC LIMIT 9");
	...
	<section class="section" id="lastSunnat">
		<div class="container">
			<h4 class="h2 d-flex justify-md-content-center justify-content-between align-items-center">
				<?php echo word('So‘ngi qo‘shilgan sunnatlar') ?>
				<span style="font-size:16px">
					<a href="sunnat">
						<span class="d-none d-md-block">
							<?php echo word('Barchasini ko‘rish')?></span>
			...
			<div class="row">
				<?php $lastSunnat = mysqli_query($db, "SELECT * FROM sunnatlar ORDER BY id DESC LIMIT 9");
